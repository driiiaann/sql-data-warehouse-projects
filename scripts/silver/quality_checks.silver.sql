...
Target tables:
    ...
    - bronze.erp_loc_a101
Purpose:
    Run “expect no results” checks for:
 ...
 *    - sls_ord_num should be NOT NULL and unique.
 *    - Add similar checks for other primary keys as needed.
 ******************************************************************************/

/* 1.1 crm_sales_details: null or duplicate sls_ord_num */
SELECT
    sls_ord_num,
    COUNT(*) AS cnt
...
/* 2.5 erp_cust_az12: customer id and gender with extra spaces */
SELECT cid, gen
FROM bronze.erp_cust_az12
WHERE (cid IS NOT NULL AND cid <> TRIM(cid))
   OR (gen IS NOT NULL AND gen <> TRIM(gen));

/* 2.6 erp_loc_a101: country codes with extra spaces */
SELECT cid, cntry
...
/******************************************************************************
 * 4. INVALID DATES AND DATE ORDERS (INTEGER YYYYMMDD)
 *    Expectation: no rows returned
 *    - Dates stored as 8‑digit integers (YYYYMMDD) in bronze.
 *    - Check for null/zero, wrong length, and out-of-range values.
 *    - Then check logical ordering in silver where dates are real DATE types.
 ******************************************************************************/

/* 4.1 crm_sales_details: raw order dates that are null, zero, wrong length, or out of range */
...
/* 4.2 crm_sales_details: raw ship dates invalid */
...
/* 4.3 crm_sales_details: raw due dates invalid */
...
/* 4.4 silver layer (after conversion to DATE): invalid date ordering
       - ship date before order date
       - due date before order date
       - ship date after due date
   Expectation: no rows
*/
SELECT
    sls_ord_num,
 ...
 *    - Negative or zero quantities/prices where not expected
 ******************************************************************************/

/* 5.1 crm_sales_details: negative or null quantities/prices/sales */
SELECT
    sls_ord_num,
    sls_quantity,
    sls_price,
...
/* 5.2 crm_sales_details: sales not equal to quantity * price */
SELECT
    sls_ord_num,
    sls_quantity,
    sls_price,
...
/* 5.3 crm_sales_details: sign mismatch between sales and price */
SELECT
    sls_ord_num,
    sls_quantity,
    sls_price,
 ...
 *    Not strictly data-quality rules, but useful for monitoring.
 ******************************************************************************/

/* 6.1 Total row counts per key table */
